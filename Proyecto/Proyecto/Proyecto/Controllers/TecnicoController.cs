...
namespace Proyecto.Controllers
{
    ...
    public class TecnicoController : Controller
    {
        ...
        public TecnicoController(AppDbContext context, EmailService emailService)
        {
            _context = context;
        ...
        {
            int tecnicoId = HttpContext.Session.GetInt32("UsuarioId") ?? 0;

            var ticketsAsignados = _context.Asignacion
                .Include(a => a.Ticket)
        ...
        // Ver detalles de un ticket asignado
        ...
        public IActionResult ActualizarEstado(int id, string estado)
        {
            ...
            if (usuario != null)
            {
                string asunto = $"Actualización de estado del ticket #{ticket.Id}";
                string cuerpo = $"Hola {usuario.NombreUsuario},<br/><br/>" +
                                $"Tu ticket con ID <strong>#{ticket.Id}</strong> ha sido actualizado.<br/>" +
                                $"<strong>Estado anterior:</strong> {estadoAnterior}<br/>" +
                                $"<strong>Nuevo estado:</strong> {estado}<br/>" +
                                $"<strong>Fecha de cambio:</strong> {DateTime.Now}<br/><br/>" +
                                $"Puedes ingresar al sistema para ver más detalles.<br/><br/>" +
        ...
        public IActionResult MisTicketsCreados()
        {
            var usuarioId = HttpContext.Session.GetInt32("UsuarioId") ?? 0;

            var tickets = _context.Ticket
                .Include(t => t.Categoria)
                .Where(t => t.CreadoPorId == usuarioId)
        ...
        public IActionResult DetalleTicketCreador(int id)
        {
            var usuarioId = HttpContext.Session.GetInt32("UsuarioId") ?? 0;
            ...
            var ticket = _context.Ticket
                ...
                .Include(t => t.Comentarios).ThenInclude(c => c.Usuario)
                .Include(t => t.Adjuntos)
                .FirstOrDefault(t => t.Id == id && t.CreadoPorId == usuarioId);
